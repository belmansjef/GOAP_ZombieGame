...
Elite::Vector2 GOAP::Action_SearchArea::GetNextCorner() const
{
	...
	{
	case Corner::TopLeft:
		return { m_pHouse->Location.x - m_pHouse->Size.x / 2.f - 13.5f, m_pHouse->Location.y + m_pHouse->Size.y / 2.f + 13.5f };
	case Corner::TopRight:
		return { m_pHouse->Location + m_pHouse->Size / 2.f + Elite::Vector2{13.5f, 13.5f} };
	case Corner::BottomRight:
		return { m_pHouse->Location.x + m_pHouse->Size.x / 2.f + 13.5f, m_pHouse->Location.y - m_pHouse->Size.y / 2.f - 13.5f };
	case Corner::BottomLeft:
		return { m_pHouse->Location - m_pHouse->Size / 2.f - Elite::Vector2{13.5f, 13.5f} };
