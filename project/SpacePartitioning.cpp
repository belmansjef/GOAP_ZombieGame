...
Cell CellSpace::GetNextCellExpandingSquare(const Elite::Vector2& agentPos)
{
	int row{ m_NrOfRows / 2 - 1};
	int col{ m_NrOfCols / 2 - 1};
	float closestDist{ FLT_MAX };
	Cell closestCell{ m_Cells[0] };

	for (int size = 1; size < m_NrOfCols; size += 6	)
	{
		// Right
		for (int i = 0; i < (size == 7 ? size * 2 : size); i++)
		{
			Cell currentCell = m_Cells[row * m_NrOfCols + col];
			const float dist{ currentCell.center.DistanceSquared(agentPos) };
			if (!currentCell.hasVisited && dist < closestDist)
			{
		...
		// left
		for (int i = 0; i < (size == 7 ? (size + 3) * 2 : size + 3); i++)
		{
			Cell currentCell = m_Cells[row * m_NrOfCols + col];
			const float dist{ currentCell.center.DistanceSquared(agentPos) };
			if (!currentCell.hasVisited && dist < closestDist)
